...
(function() {
  ...
  let terminal_event = () => {
    ...
    let promise_parse_list = (mobj) => new Promise((resolve, reject) => {
      if (mobj._.length >= 1 && mobj._[0] == "list") {
        if (mobj._[1] == "bytag") {
          json["undone"] = _.sortBy(json["undone"], (k:string) => {
  ...
  promise_json_load("todo/20160628192500.json")
    .then(terminal_event);
...
function update_feed() {
  ...
  let feedHTML = json => {
    let uditems = _.map(json["undone"], (k :string) => json["items"][k]);
  ...
  let uiacc: any = $('.ui.accordion'); uiacc.accordion();
  let uidrop: any = $('.ui.dropdown'); uidrop.dropdown();
  let datetp: any = $('.datetimepicker'); datetp.datetimepicker({
    format: 'Y-m-d H:i:s',
    step: 30
  });
