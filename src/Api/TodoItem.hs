...
newtype ItemDTO = ItemDTO (T.Text, Maybe UTCTime, T.Text, T.Text, T.Text, [T.Text], TodoListId)
...
serverTodoItemAPI = getAll :<|> getById :<|> postNew :<|> updateItem where
  ...
  postNew json = case fromJSON @ItemDTO json of
    ...
    Success (ItemDTO (name, due, icon, color, msg, tgs, listid)) -> do
      time <- lift getCurrentTime
      fmap Just $ lift $ insertTodoItem (TodoItem name time due icon color msg tgs listid)
